	...
	<view>
		<view v-if="!hid" class="flex-row-center" :style="{ top: scHight }" style="width: 750rpx; position: fixed; z-index: 100; left: 0">
			<view class="flex-column-center" style="background-color: #fcfcfc; padding: 30rpx; border-radius: 10rpx">
				...
				<movable-area class="flex-row-start" style="width: 100%; background-color: #efefef; height: 80rpx; border-radius: 40rpx; margin-top: 30rpx">
					<movable-view
						scale-value="1"
						animation="false"
						damping="50"
						...
						@touchend="end"
					>
						<u-icon :color="mainColor" size="40" v-if="endLoad" name="arrow-right"></u-icon>
						<u-icon :color="mainColor" size="40" v-else name="reload"></u-icon>
					</movable-view>

					<text style="padding-left: 140rpx" :style="{ color: col }">{{ hasImg }}</text>
				</movable-area>
				<view class="flex-row-around padding-top" style="width: 100%">
					<u-icon @click="hide" :color="mainColor" size="40" name="close"></u-icon>
...
export default {
	...
	methods: {
		...
		getCode() {
			...
			//测试
			this.key = "123456";
			this.$store.state.verificationKey = "123456";
			...
			console.log('touch-verification');
			this.endLoad = false;
			// 验证拼图位置是否正确
