	...
	<div class="wrapper">
		<div v-if="!wechatLogin">
			...
			<div v-show="!enableUserPwdBox">
				<div v-show="current == 0">
					<u-input
						...
						maxlength="11"
					/>
					<div :class="!enableFetchCode ? 'disable' : 'fetch'" @click="fetchCode" class="btn">获取验证码</div>
				</div>
				<!-- 输入验证码 -->
				<div v-show="current == 1" class="box-code">
					<verifyCode type="bottom" @confirm="save" boxActiveColor="#D8D8D8" v-model="code" isFocus boxNormalColor="#D8D8D8" cursorColor="#D8D8D8" />
			...
			<div v-show="enableUserPwdBox">
				...
				<div :class="!enableUserBtnColor ? 'disable' : 'fetch'" @click="passwordLogin" class="btn">帐号密码登录</div>
			</div>
			...
			<div class="flex" v-show="current != 1">
				...
				<div class="tips">
					...
					<span @click="navigateToPrivacy('privacy')">《隐私协议》</span>
					<span @click="navigateToPrivacys('user')">《用户协议》</span>
				</div>
			</div>

			<div v-if="current != 1" class="user-password-tips" @click="enableUserPwdBox = !enableUserPwdBox">{{ !enableUserPwdBox ? '帐号密码' : '手机号' }}登录</div>
...
export default {
	...
	watch: {
		...
		async flage(val) {
			console.log('login-flage', val, this.$refs.uCode.canGetCode, this.enableUserPwdBox);
			if (val) {
