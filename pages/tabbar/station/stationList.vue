<template>
	<view class="wrapper">
		...
		<view class="swiper-wrap">
			<u-swiper :list="advertisingMap" duration="1000" interval="5000" :circular="true" :effect3d="true"
				img-mode="aspectFit" height="300" border-radius="40" bg-color="#D7DEEB"></u-swiper>
		</view>
		...
		<scroll-view scroll-y class="list">
			<view v-for="(station, cindex) in stationList" :key="cindex" class="station-card"
				...
				<view class="tags">
					<view v-for="(t, index) in station.tagList" :key="index" class="tag"
						style="margin: 10rpx 10rpx 0 0">{{t.tagName}}</view>
				</view>
				<view class="bottom">
					...
					<view class="right">
						<view class="label fast" v-if="station.totalFastGun>0">空闲
							{{ station.freeFastGun }}/{{ station.totalFastGun }}
						</view>
		...
		methods: {
			...
			async getStationList() {
				...
				if (true) {
					...
					_this.stationList.forEach((station, index) => {
						...
						labelContent += station.totalFastGun > 0 ?
							`快${station.freeFastGun}/${station.totalFastGun} ` : ' '
						labelContent += (station.totalGun - station.totalFastGun) > 0 ?
							`慢${station.freeGun - station.freeFastGun}/${station.totalGun - station.totalFastGun}` :
							''
			...
			calUserDistance() {
				this.stationList.forEach((p, i) => {
					...
					let userDistance = Math.round(this.$options.filters.calcDistance(this.userLocation.lon, this
						.userLocation.lat, lon, lat) / 100) / 10;
