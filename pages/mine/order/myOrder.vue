<template>
	<div class="wrapper">
		<view v-for="(order, oIndex) in list" :key="oIndex" class="orderCard" @click="handleClick(order)">
			...
			<view class="address">充电方式：自动充电</view>
			<view class="address">下单时间：{{ order.startTm }}</view>
			<view class="bottom">
...
<script>
import { myOrder } from '@/config/virtualData.js';
...
export default {
	...
	methods: {
		...
		async init() {
			...
			let res = { data: { code: 200, data: {} } }; //测试数据
			res.data.data = myOrder.orderListVo;
			console.log(res);
