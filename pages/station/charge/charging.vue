					...
					<view style="margin-top: 200rpx">
						<view style="margin-top: 150rpx; padding: 30rpx 20rpx; display: flex; color: #fff">
							<u-circle-progress
								...
								width="80"
								:percent="equipmentInfo.soc"
								:show="orderInfo.status == 3"
							></u-circle-progress>
							<view style="flex: 1">
						...
						<!-- 参数 -->
						<view class="card-swapper" v-if="equipmentInfo.status == 1 || equipmentInfo.status == 3 || (equipmentInfo.status == 4 && equipmentInfo.orderStatus == 10)">
							<u-row gutter="8">
								<u-col span="12">
									<view class="label">
										<view class="number">
				...
				<view v-if="orderInfo.status == 99">
					...
					<view class="card-box" style="padding: 30rpx 20rpx">
						...
						<view class="card-row">
							...
							<view class="content">￥{{ equipmentInfo.totalMoney | unitPrice }}</view>
						</view>
						<view class="card-row">
							<view class="lable"></view>
							<view class="content">
								实际支付
								<text class="actual-pay">￥{{ equipmentInfo.totalMoney | unitPrice }}</text>
							</view>
						</view>
					</view>
					<view class="card-box" style="padding: 30rpx 20rpx">
...
<script>
import { chargingP, stationP, stationIndexP } from '@/config/virtualData.js';
/************接口API***************/
...
export default {
	...
	methods: {
		...
		async init(id) {
			...
			response.data.data = chargingP.PrepareChargingVo;
			...
			if (response.data.code == 401) {
				...
				let stationRes = { data: { data: {}, code: 200 } };
				stationRes.data.data = stationIndexP;
		...
		async getChargeInfo() {
			...
			if (res.data.code == 200 && res.data.data != null) {
				 ...
				 */
				if (
					this.equipmentInfo.status == 4 &&
					this.equipmentInfo.orderStatus == 30 &&
					this.equipmentInfo.billInfo.occupyPrice > 0 &&
					this.equipmentInfo.billInfo.occupyFlag == 1
				) {
...
.product-container {
	background-image: linear-gradient(180deg, #0593d5 12%, #00a5ff 18%, #f7f7f7 44%);
...
@media (prefers-color-scheme: dark) {
	.product-container {
		background-image: linear-gradient(180deg, #039ef1 12%, #056d9e 18%, #121425 40%);
	}
